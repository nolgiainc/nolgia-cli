...
**Any Linux works, including slim containers.** The Linux binaries are statically linked and need no system library, so `debian:bookworm-slim`, `ubuntu:22.04` and `alpine` are all fine. Your image does need a CA certificate bundle to reach the API over HTTPS — if you installed with `curl` you already have one; on a bare image install `ca-certificates`. These binaries carry no OS keyring, which changes nothing for you: the token is stored in a `0600` file by default.
