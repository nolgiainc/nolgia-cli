...
**Check your platform first — we know one case that fails.** The Linux release binaries need **glibc 2.38 or newer** and the `libdbus-1.so.3` library. If you are inside a slim container (`debian:bookworm-slim`, `ubuntu:22.04`, most CI base images), the download will succeed and the binary **will not start**. Run `ldd --version | head -1`: if it reports less than 2.38, skip to the Cargo fallback below instead, which builds against your own system. On such a machine the installer now fails loudly with the loader error rather than reporting a success you cannot use. This is tracked in NOL-1070; macOS is unaffected.
